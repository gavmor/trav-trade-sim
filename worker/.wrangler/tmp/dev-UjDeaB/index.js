...
// src/lib/session.js
var TTL_DAYS = 30;
async function createSession(db, playerId, campaignId) {
...
async function requireReferee(c, next) {
  const header = c.req.header("Authorization") ?? "";
  const token = header.startsWith("Bearer ") ? header.slice(7) : null;
  const session = await verifySession(c.env.DB, token);
  if (!session) return c.json({ error: "Unauthorized" }, 401);
...
__name(requireReferee, "requireReferee");
...
app.post("/", async (c) => {
  ...
  await db.batch([
    db.prepare(`INSERT INTO campaigns (id, code, label, milieu, trade_rules, recovery_code_hash) VALUES (?, ?, ?, ?, ?, ?)`).bind(campaignId, code.trim().toUpperCase(), label.trim(), milieu ?? "M1105", trade_rules ?? "CT7", recoveryHash),
    db.prepare(`INSERT INTO campaign_calendar (campaign_id, current_tick, year, day) VALUES (?, ?, ?, ?)`).bind(campaignId, tick, year, day),
    db.prepare(`INSERT INTO players (id, campaign_id, character_name, pin_hash, role) VALUES (?, ?, ?, ?, 'referee')`).bind(playerId, campaignId, char_name.trim(), pinHash)
  ]);
...
app.post("/join", async (c) => {
  ...
  const playerId = crypto.randomUUID();
  const pinHash = await hashPin(pin);
  await db.prepare(`INSERT INTO players (id, campaign_id, character_name, pin_hash, role) VALUES (?, ?, ?, ?, 'player')`).bind(playerId, campaign.id, char_name.trim(), pinHash).run();
...
app.post("/recovery-code", requireReferee, async (c) => {
  ...
  const recoveryCode = crypto.randomUUID().toUpperCase().replace(/-/g, "");
  const recoveryHash = await hashPin(recoveryCode);
  await c.env.DB.prepare(`UPDATE campaigns SET recovery_code_hash = ? WHERE id = ?`).bind(recoveryHash, session.campaign_id).run();
...
app2.post("/login", async (c) => {
  ...
  if (!player) return c.json({ error: "Character not found" }, 404);
  if (player.locked_until && new Date(player.locked_until) > /* @__PURE__ */ new Date()) {
  ...
  const lockedUntil = newAttempts >= 5 ? new Date(Date.now() + 15 * 6e4).toISOString() : null;
  await db.prepare(
...
app2.post("/logout", requireAuth, async (c) => {
  const header = c.req.header("Authorization") ?? "";
  const token = header.startsWith("Bearer ") ? header.slice(7) : null;
...
// src/lib/rollup.js
function tickYear(tick) {
  return 1105 + Math.floor(tick / 48);
}
__name(tickYear, "tickYear");
function tickMonth(tick) {
  return Math.floor(tick / 4) % 12 + 1;
}
__name(tickMonth, "tickMonth");
function tickDay(tick) {
  return tick % 48 * 7 + 1;
}
__name(tickDay, "tickDay");
...
__name(doRollupMonth, "doRollupMonth");
...
__name(doRollupYear, "doRollupYear");
async function advanceTick(db, campaignId) {
...
__name(advanceTick, "advanceTick");

// src/routes/calendar.js
var app3 = new Hono2();
app3.get("/:id/calendar", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  if (session.campaign_id !== id) return c.json({ error: "Forbidden" }, 403);
...
app3.post("/:id/advance-tick", requireReferee, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  if (session.campaign_id !== id) return c.json({ error: "Forbidden" }, 403);
...
var calendar_default = app3;

// src/routes/market.js
var app4 = new Hono2();
app4.get("/:id/events", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app4.post("/:id/events", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  if (session.campaign_id !== id) return c.json({ error: "Forbidden" }, 403);
  const body = await c.req.json();
  if (body.check_duplicate) {
    ...
    return c.json({ data: { count: row2?.cnt ?? 0 } });
  }
...
app4.patch("/event/:eventId/expire", requireReferee, async (c) => {
...
app4.get("/:id/snapshots", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const { world_hex, sector, tick, count: count3 } = c.req.query();
  if (session.campaign_id !== id) return c.json({ error: "Forbidden" }, 403);
  const db = c.env.DB;
  if (count3 === "true") {
    const row = await db.prepare(
...
app4.get("/:id/snapshots/prior-count", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app4.post("/:id/snapshots", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  if (session.campaign_id !== id) return c.json({ error: "Forbidden" }, 403);
...
app4.get("/:id/market/weekly", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app4.get("/:id/market/monthly", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app4.get("/:id/market/realized", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app4.get("/:id/market/annual", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const { world_hex, sector, good_die } = c.req.query();
  if (session.campaign_id !== id) return c.json({ error: "Forbidden" }, 403);
...
var market_default = app4;

// src/routes/ships.js
var app5 = new Hono2();
app5.get("/current", requireAuth, async (c) => {
  const session = c.var.session;
  ...
  const ship = {
    id: crew.id,
    name: crew.name,
    hull_type: crew.hull_type,
    hull_tons: crew.hull_tons,
    cargo_capacity: crew.cargo_capacity,
    current_world: crew.current_world,
    current_sector: crew.current_sector,
    credits: crew.credits,
    jump_rating: crew.jump_rating,
    ...
    fuel_capacity: crew.fuel_capacity,
    fuel_current: crew.fuel_current,
    crew_role: crew.role,
    can_trade: crew.can_trade === 1
  };
...
app5.post("/", requireAuth, async (c) => {
  ...
  await db.batch([
    db.prepare(`INSERT INTO ships (id, campaign_id, name, hull_type, hull_tons, cargo_capacity) VALUES (?, ?, ?, ?, ?, ?)`).bind(shipId, campaign_id, name, hull_type ?? null, hull_tons ?? 200, cargo_capacity ?? 80),
    db.prepare(`INSERT INTO crew (id, campaign_id, ship_id, player_id, role, can_trade, joined_tick) VALUES (?, ?, ?, ?, 'captain', 1, ?)`).bind(crewId, campaign_id, shipId, player_id, current_tick)
  ]);
...
app5.patch("/:id", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app5.patch("/:id/credits", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app5.post("/:id/buy-cargo", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  ...
  await db.batch([
    db.prepare(`INSERT INTO cargo (id, campaign_id, player_id, ship_id, trade_good_die, trade_good_name, tons, purchase_price, purchased_tick, purchase_world, purchase_world_name, purchase_sector)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(cargoId, campaign_id, player_id, id, good.trade_good_die, good.trade_good_name, tons, good.purchase_price, tick, world_hex, world_name ?? "", sector),
    db.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, trade_good_die, trade_good_name, tons, price_per_ton, total_cr, world_hex, sector)
                VALUES (?, ?, ?, ?, ?, 'buy', ?, ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), campaign_id, player_id, id, tick, good.trade_good_die, good.trade_good_name, tons, good.purchase_price, -totalCost, world_hex, sector),
    db.prepare(`UPDATE ships SET credits = credits - ? WHERE id = ?`).bind(totalCost, id),
    db.prepare(`UPDATE market_snapshots SET qty_available = qty_available - ? WHERE campaign_id = ? AND world_hex = ? AND sector = ? AND trade_good_die = ? AND tick = ?`).bind(tons, campaign_id, world_hex, sector, good.trade_good_die, tick)
  ]);
  const cargoRow = await db.prepare(`SELECT * FROM cargo WHERE id = ?`).bind(cargoId).first();
  return c.json({ data: cargoRow }, 201);
});
app5.post("/:id/sell-cargo", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, trade_good_die, trade_good_name, tons, price_per_ton, total_cr, world_hex, sector)
                      VALUES (?, ?, ?, ?, ?, 'sell', ?, ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), campaign_id, cargo_item.player_id, id, tick, cargo_item.trade_good_die, cargo_item.trade_good_name, cargo_item.tons, sell_price_per_ton, totalRevenue, market_world_hex, market_sector),
...
app5.post("/:id/book-passengers", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const body = await c.req.json();
  const {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`INSERT INTO passenger_manifests (id, campaign_id, ship_id, player_id, passage_type, count, embark_world_hex, embark_sector, embark_world_name, embark_tick, dest_world_hex, dest_sector, dest_world_name, fare_per_head, fare_total)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(manifestId, campaign_id, id, player_id, passage_type, count3, embark_world_hex, embark_sector, embark_world_name ?? "", tick, dest_world_hex, dest_sector, dest_world_name ?? "", fare_per_head, fare_total),
    c.env.DB.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, total_cr, world_hex, sector, notes)
                      VALUES (?, ?, ?, ?, ?, 'passenger_fare', ?, ?, ?, ?)`).bind(
      crypto.randomUUID(),
      campaign_id,
      player_id,
      id,
      ...
      embark_world_hex,
      embark_sector,
      `${count3}\xD7 ${passage_type} \u2192 ${dest_world_name || dest_world_hex}`
    ),
...
app5.post("/:id/deliver-passengers", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  ...
  const stmts = ids.map(
    (pid2) => c.env.DB.prepare(`UPDATE passenger_manifests SET status = 'delivered', resolve_tick = ? WHERE id = ? AND campaign_id = ?`).bind(tick, pid2, campaign_id)
  );
  await c.env.DB.batch(stmts);
  return c.json({ data: { ok: true } });
});
app5.post("/:id/refund-passenger", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  ...
  await db.batch([
    ...
    db.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, total_cr, world_hex, sector, notes)
      ...
      tick,
      -manifest.fare_total,
      manifest.embark_world_hex,
      manifest.embark_sector,
      `Refund: ${manifest.count}\xD7 ${manifest.passage_type}`
    ),
    db.prepare(`UPDATE ships SET credits = credits - ? WHERE id = ?`).bind(manifest.fare_total, id)
  ]);
  return c.json({ data: { ok: true } });
});
app5.post("/:id/purchase-fuel", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, tons, price_per_ton, total_cr, world_hex, sector, notes)
                      VALUES (?, ?, ?, ?, ?, 'fuel', ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), campaign_id, player_id, id, tick, tons, price_per_ton, -totalCost, world_hex, sector, `${fuel_type} fuel`),
...
app5.post("/:id/accept-mail", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const {
    campaign_id,
...
app5.post("/:id/deliver-mail", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
app5.get("/:id/passengers", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
...
var ships_default = app5;

// src/routes/referee.js
var app6 = new Hono2();
app6.get("/ships", requireReferee, async (c) => {
  const session = c.var.session;
  ...
  const ships = (shipRows ?? []).map((s) => ({
    ...s,
    crew: (crewRows ?? []).filter((c2) => c2.ship_id === s.id).map((c2) => ({
      id: c2.id,
      ship_id: c2.ship_id,
      role: c2.role,
      can_trade: c2.can_trade === 1,
      has_stateroom: c2.has_stateroom === 1,
      joined_tick: c2.joined_tick,
      players: { id: c2.player_id, character_name: c2.character_name, role: c2.player_role }
    }))
  }));
  return c.json({ data: ships });
});
app6.post("/ships", requireReferee, async (c) => {
  const session = c.var.session;
  const {
    name,
    hull_type,
...
app6.patch("/ships/:id", requireReferee, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const fields = await c.req.json();
  const db = c.env.DB;
...
app6.post("/crew", requireReferee, async (c) => {
  const session = c.var.session;
...
app6.get("/players", requireReferee, async (c) => {
  const session = c.var.session;
  const campaignId = session.campaign_id;
  const db = c.env.DB;
...
app6.post("/skills", requireReferee, async (c) => {
  const session = c.var.session;
...
app6.delete("/skills/:id", requireReferee, async (c) => {
  const { id } = c.req.param();
  await c.env.DB.prepare(`DELETE FROM player_skills WHERE id = ?`).bind(id).run();
  return c.json({ data: { ok: true } });
});
app6.post("/ships/:id/refund-passenger", requireReferee, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const { manifest_id, tick, campaign_id, player_id } = await c.req.json();
  if (campaign_id !== session.campaign_id) return c.json({ error: "Forbidden" }, 403);
...
app6.post("/ships/:id/auto-deliver", requireReferee, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  ...
  const stmts = [
    ...(passengers ?? []).map(
      (p) => db.prepare(`UPDATE passenger_manifests SET status = 'delivered', resolve_tick = ? WHERE id = ?`).bind(tick, p.id)
    ),
    ...(passengers ?? []).map(
      (p) => db.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, total_cr, world_hex, sector, notes)
                  VALUES (?, ?, ?, ?, ?, 'passenger_fare', 0, ?, ?, ?)`).bind(
        crypto.randomUUID(),
        campaign_id,
        p.player_id,
        id,
        ...
        world_hex,
        sector,
        `Delivered: ${p.count}\xD7 ${p.passage_type} from ${p.embark_world_name || p.embark_world_hex}`
      )
    ),
    ...(mail ?? []).map(
      (m) => db.prepare(`UPDATE mail_contracts SET status = 'delivered', resolve_tick = ? WHERE id = ?`).bind(tick, m.id)
    ),
    ...(mail ?? []).map(
      (m) => db.prepare(`INSERT INTO transactions (id, campaign_id, player_id, ship_id, tick, type, total_cr, world_hex, sector, notes)
                  VALUES (?, ?, ?, ?, ?, 'mail', ?, ?, ?, ?)`).bind(
        crypto.randomUUID(),
        campaign_id,
        player_id,
...
var referee_default = app6;

// src/routes/reports.js
var app7 = new Hono2();
app7.get("/ledger", requireAuth, async (c) => {
  const session = c.var.session;
  ...
  if (from_tick != null) {
    sql += ` AND tick >= ?`;
    args.push(Number(from_tick));
  }
  if (to_tick != null) {
    sql += ` AND tick < ?`;
    args.push(Number(to_tick));
  }
...
app7.get("/trades", requireAuth, async (c) => {
  const session = c.var.session;
  const { ship_id, limit = "101", from_tick, to_tick } = c.req.query();
...
app7.get("/income", requireAuth, async (c) => {
  const session = c.var.session;
...
app7.post("/skills", requireAuth, async (c) => {
  const session = c.var.session;
...
app7.get("/skills", requireAuth, async (c) => {
  const session = c.var.session;
...
app7.delete("/skills/:id", requireAuth, async (c) => {
  const session = c.var.session;
  const { id } = c.req.param();
  const row = await c.env.DB.prepare(
...
app8.get("/", (c) => c.json({ ok: true, service: "trav-trade-sim" }));
app8.route("/api/campaigns", campaigns_default);
app8.route("/api/auth", auth_default);
app8.route("/api/campaigns", calendar_default);
app8.route("/api/campaigns", market_default);
app8.route("/api/ships", ships_default);
app8.route("/api/referee", referee_default);
app8.route("/api/reports", reports_default);
...
// .wrangler/tmp/bundle-TPcBoA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TPcBoA/middleware-loader.entry.ts
